...
async function whatsappConnect() {
    ...
    if (connection === 'close') {
        ...
        console.log(chalk.bold.white("Simple WhatsApp bot Made by Mahes")) // Hargailah yang create jangan ditempel² weem anj
        }
        ...
        for (let n of participants) {
            ...
            switch (action) {
             ...
             case 'remove': {
                ...
                shyzu.sendMessage(id, { text: (action === 'add' ? t : t2) }, { contextInfo: { mentionedJid: [n], externalAdReply: { title: action === 'add' ? 'Welcome' : 'Goodbye', body: "© Created by Mahes", thumbnailUrl: ppser, mediaType: 3, renderLargerThumbnail: false }}, mentions: [n] },{})
             }
             break;
             case 'promote': {
                var u = shyzu.promote.replace("user", n.split("@")[0])
                shyzu.sendMessage(id, { text: u }, { contextInfo: { mentionedJid: [n], externalAdReply: { title: '', body: "© Created by Mahes", thumbnailUrl: ppser, mediaType: 3, renderLargerThumbnail: false }}, mentions: [n] },{})
             }
             break;
             case 'demote': {
